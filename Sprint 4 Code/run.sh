...
setup_backend() {
  ...
  # backend/.env is gitignored, so a fresh clone has none and prisma.config.ts
  # fails to resolve DATABASE_URL. Seed it with the same default src/prisma.ts uses.
  if [[ ! -f "$ROOT_DIR/backend/.env" ]]; then
    echo "Creating backend/.env with default DATABASE_URL..."
    echo 'DATABASE_URL="file:./dev.db"' >"$ROOT_DIR/backend/.env"
  fi
...
setup_py() {
  ...
  if [[ "$current_hash" != "$previous_hash" ]]; then
    echo "Installing python requirements (optional tooling)..."
    if "$ROOT_DIR/venv/bin/python" -m pip install --default-timeout=15 --retries=2 -r requirements.txt; then
      write_marker "$marker" "$current_hash"
    else
      echo "WARNING: PyPI network download timed out. Python optional tooling skipped for now."
      write_marker "$marker" "$current_hash"
    fi
  else
