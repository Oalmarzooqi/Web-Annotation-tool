...
/** Physical voxel size in micrometres, when the file declares it. */
type Spacing = { umPerPxLateral: number; umPerPxAxial: number; umPerSlice: number };
type ReadyResp = {
  type: "ready";
  sliceCount: number;
  width: number;
...
/**
 * Physical voxel size from the standard tags, in micrometres.
 *
 * PixelSpacing (0028,0030) is "rowSpacing\\colSpacing" in mm: row spacing steps down
 * the image (axial/depth), column spacing steps across it (lateral). Slice spacing
 * prefers SpacingBetweenSlices (0018,0088) and falls back to SliceThickness
 * (0018,0050). Returns null unless the in-plane pair is present and positive —
 * a guessed scale is worse than an obviously-default one.
 */
function parseSpacing(dataSet: dicomParser.DataSet): Spacing | null {
  try {
    const ps = dataSet.string("x00280030");
    if (!ps) return null;
    const parts = ps.split("\\");
    const rowMm = parseFloat(parts[0] ?? "");
    const colMm = parseFloat(parts[1] ?? parts[0] ?? "");
    if (!Number.isFinite(rowMm) || !Number.isFinite(colMm) || rowMm <= 0 || colMm <= 0) {
      return null;
    }
    let sliceMm = numFromDs(dataSet, "x00180088", 0);
    if (!(sliceMm > 0)) sliceMm = numFromDs(dataSet, "x00180050", 0);
    return {
      umPerPxAxial: rowMm * 1000,
      umPerPxLateral: colMm * 1000,
      // No slice spacing declared: fall back to the in-plane lateral pitch rather
      // than 0, which would collapse the map's physical height.
      umPerSlice: sliceMm > 0 ? sliceMm * 1000 : colMm * 1000,
    };
  } catch {
    return null;
  }
...
function parseFrameMeta(dataSet: dicomParser.DataSet): FrameMeta {
  ...
  const spacing = parseSpacing(dataSet);
  ...
  try {
    ...
    if (msg.type === "init") {
      ...
      const resp: ReadyResp = {
        ...
        width: meta.cols,
        height: meta.rows,
        ...(meta.spacing ? { spacing: meta.spacing } : {}),
      };
