...
/** TIFF tag values arrive as single-element arrays (e.g. t256: [1000]). */
function tagNumber(v: unknown): number {
  const raw = Array.isArray(v) ? v[0] : v;
  return getNumber(raw, 0);
}
...
self.onmessage = async (ev: MessageEvent<Msg>) => {
  try {
    ...
    if (msg.type === "init") {
      ...
      const first = ifds[0] as { width?: unknown; height?: unknown; t256?: unknown; t257?: unknown };
      // UTIF.decode() only reads the directory: `width`/`height` stay undefined
      // until decodeImage() runs per slice. The size tags are already there, so
      // read those — otherwise `ready` reports 0x0 for every TIFF.
      w = getNumber(first.width, tagNumber(first.t256));
      h = getNumber(first.height, tagNumber(first.t257));
