...
import { OctCanvas, OctLabelPanel, OctToolbar, ShortcutsDialog } from "../../components/oct";
import {
  parseProjectData,
...
import type {
  ...
  OctCanvasFrame,
  ZoomCommand,
} from "../../components/oct/OctCanvas";
import { setAnnotationPointAt, type EditHit } from "../../lib/annotationEdit";
import { LAYER_MIN_POINTS } from "../../lib/layerLine";
import { SPLINE_MIN_POINTS, collectSplines, nextSplineName } from "../../lib/spline";
...
type WorkerReady = {
  ...
  width: number;
  height: number;
  /** Physical voxel size, when the file declares it (DICOM PixelSpacing). */
  spacing?: VolumeScale;
};
type WorkerSlice = {
  type: "slice";
...
async function createStackVolumeFromBuffer(
  ...
  return {
    ...
    width: ready.width,
    height: ready.height,
    ...(ready.spacing ? { spacing: ready.spacing } : {}),
  };
}
...
export function ProjectAnnotatePage() {
  ...
  /**
   * Geometry of the loaded volume, recorded into the project on save so the
   * Measurements view can size a thickness map without decoding the image again.
   * Declared here (not beside `sliceCount`) because saveProjectToDb closes over it.
   */
  const volumeDims = useMemo<FileDims | null>(() => {
    if (!volume) return null;
    const d: FileDims =
      volume.kind === "single"
        ? { width: volume.frame.width, height: volume.frame.height, sliceCount: 1 }
        : {
            width: volume.width,
            height: volume.height,
            sliceCount: volume.sliceCount,
            ...(volume.spacing ? { spacing: volume.spacing } : {}),
          };
    // Never persist a zero dimension — it would read back as "unknown" anyway,
    // and a half-filled record is harder to diagnose than none at all.
    return d.width > 0 && d.height > 0 && d.sliceCount > 0 ? d : null;
  }, [volume]);
  ...
  /** Serialized payload of the last successful save — auto-save skips no-ops. */
  ...
  /** The v2 multi-file map only — parsing itself lives in lib/projectData. */
  function parseAnnotationMap(
    raw: string | null | undefined,
  ): Record<string, Record<number, Annotation[]>> {
    return parseProjectData(raw).files;
  }

  /** Everything auto-save watches, as one comparable string. */
  const savePayload = useMemo(
    () => JSON.stringify({ a: history.present.annotationsBySlice, l: labels, d: volumeDims }),
    [history.present.annotationsBySlice, labels, volumeDims],
  );

  const saveProjectToDb = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      if (!project) {
        if (!silent) showToast("Cannot save to cloud: No active project loaded.", "error");
        return;
      }
      // Nothing changed since the last write (also stops the initial DB load
      // from echoing straight back as a save). A null baseline means the load
      // has not settled yet — never let a background save write over it.
      if (silent && (lastSavedRef.current === null || lastSavedRef.current === savePayload)) return;

      const fileKey = selectedFileName || "__default__";
      try {
        setSaving(true);
        setSaveSuccess(false);
        if (!silent) showToast("Saving to cloud...", "info");

        // Fetch the latest stored blob so we don't wipe sibling files — or the
        // scale/dims the Measurements view depends on. Re-serialise the WHOLE
        // object: rebuilding it from `files` alone drops them on every autosave.
        const latestProject = await import("../../lib/projects").then((m) =>
          m.getProject(project.id),
        );
        const data = parseProjectData(latestProject.annotations);
        data.files[fileKey] = history.present.annotationsBySlice;
        if (volumeDims) data.dims[fileKey] = volumeDims;

        await updateProject(project.id, {
          name: project.name,
          description: project.description,
          annotations: serializeProjectData(data),
          labels: JSON.stringify(labels),
        });
        lastSavedRef.current = savePayload;
        setSaveSuccess(true);
        if (!silent) showToast("Project annotations saved to cloud!", "success");
    ...
    [
      project,
      selectedFileName,
      history.present.annotationsBySlice,
      labels,
      savePayload,
      showToast,
      volumeDims,
  ...
  // Auto-save: 2s after the last edit. Ctrl+S still forces an immediate save.
  useEffect(() => {
    if (!project) return;
    if (lastSavedRef.current === null) {
      // First payload seen for this project — treat it as the saved baseline.
      lastSavedRef.current = savePayload;
      return;
    }
    if (lastSavedRef.current === savePayload) return;
    const t = setTimeout(() => void saveProjectToDb({ silent: true }), 2000);
    return () => clearTimeout(t);
  }, [savePayload, project, saveProjectToDb]);

  // A different project/file gets its own baseline.
  useEffect(() => {
    lastSavedRef.current = null;
  }, [id, selectedFileName]);

  // Flush a save still sitting in the 2s debounce when the view goes away
  // (file switch, leaving the page, closing the tab) — otherwise it is dropped.
  const saveRef = useRef(saveProjectToDb);
  useEffect(() => {
    saveRef.current = saveProjectToDb;
  }, [saveProjectToDb]);
  useEffect(() => {
    // Cleanup still holds the previous file's closure, which is what needs flushing.
    const flush = () => void saveRef.current({ silent: true });
  ...
  const saveAsJson = useCallback(async () => {
    ...
    const data = {
      ...
      labels,
      files: existingMap,
      // Flat, self-contained view of every named curve: one entry per spline,
      // keyed by image + frame so a consumer needs nothing else to place it.
      splines: collectSplines(existingMap, labels),
    };
  ...
  const filteredAnnotations = useMemo(() => {
    const base = showAllLayers
      ? sliceAnnotations
      : sliceAnnotations.filter((a) => a.labelId === activeLabelId);
    if (hiddenLabelIds.size === 0) return base;
    // The canvas hit-tests whatever it is given, so hidden layers drop out of
    // click, erase and edit picking for free.
    return base.filter((a) => !hiddenLabelIds.has(a.labelId));
  }, [sliceAnnotations, showAllLayers, activeLabelId, hiddenLabelIds]);
  ...
  const onCanvasClick = (p: ImagePoint) => {
    ...
    if (mode === "layer") {
      const pts = sliceDraft?.type === "layer" ? [...sliceDraft.points, p] : [p];
      setSliceDraft({ type: "layer", points: pts });
      return;
    }
    if (mode === "spline") {
      const pts = sliceDraft?.type === "spline" ? [...sliceDraft.points, p] : [p];
      setSliceDraft({ type: "spline", points: pts });
      return;
    }
    // polygon mode
  ...
  const deleteSelectedAnnotation = useCallback(() => {
    ...
    dispatchHistory({
      ...
      update: (s) => {
        ...
        return {
          ...s,
          annotationsBySlice: {
            ...s.annotationsBySlice,
            [sliceIdx]: list.filter((a) => a.id !== aid),
          },
        };
      },
    });
    setEditSelection(null);
    return true;
  }, [editSelection, sliceIdx]);

  /** Polygon needs 3 vertices to close; a layer line needs 3 points to be a curve. */
  const canFinishDraft =
    activeLabelId !== null &&
    ((mode === "polygon" && sliceDraft?.type === "polygon" && sliceDraft.points.length >= 3) ||
      (mode === "layer" &&
        sliceDraft?.type === "layer" &&
        sliceDraft.points.length >= LAYER_MIN_POINTS) ||
      (mode === "spline" &&
        sliceDraft?.type === "spline" &&
        sliceDraft.points.length >= SPLINE_MIN_POINTS));

  const finishDraft = useCallback(() => {
    if (!canFinishDraft || !activeLabelId || !sliceDraft) return;
    if (sliceDraft.type === "polygon") {
      addAnnotation({
        id: makeId(),
        labelId: activeLabelId,
        type: "polygon",
    ...
    } else if (sliceDraft.type === "spline") {
      // ponytail: window.prompt for the name — native, no dialog state to keep.
      // Swap for an inline field if renaming/validation is ever needed.
      const suggested = nextSplineName(
        (history.present.annotationsBySlice[sliceIdx] ?? [])
          .filter((a) => a.type === "spline")
          .map((a) => a.name),
      );
      const entered = window.prompt("Name this spline", suggested);
      if (entered === null) return; // cancelled — keep the draft open
      addAnnotation({
        id: makeId(),
        labelId: activeLabelId,
        type: "spline",
  ...
  /** Backspace while drafting removes the last placed point. */
  const dropLastDraftPoint = useCallback(() => {
    if (!sliceDraft) return false;
    if (sliceDraft.type === "line") return false;
    const next = sliceDraft.points.slice(0, -1);
    dispatchHistory({
      type: "commit",
      update: (s) => ({
        ...s,
        draftBySlice: {
          ...s.draftBySlice,
          [sliceIdx]:
            next.length === 0 ? null : ({ type: sliceDraft.type, points: next } as Draft),
        },
      }),
    });
    return true;
  ...
  // ── One keyboard handler, driven by the shared shortcut table ───────────
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (showShortcuts) return; // the dialog owns the keyboard while open
      ...
      switch (hit.kind) {
        case "tool":
          ...
          break;
        case "label": {
          const lab = labels[hit.index];
          if (!lab) return;
          setActiveLabelId(lab.id);
          break;
        }
        case "slice":
          if (sliceCount === 0) return;
          setSliceIdx((i) => {
            if (hit.to === "prev") return Math.max(0, i - 1);
            if (hit.to === "next") return Math.min(sliceCount - 1, i + 1);
            return hit.to === "first" ? 0 : sliceCount - 1;
          });
          break;
        case "zoom":
          setZoomCommand((c) => ({ kind: hit.to, nonce: (c?.nonce ?? 0) + 1 }));
          break;
        case "toggleActiveLabel":
          if (!activeLabelId) return;
          toggleLabelVisibility(activeLabelId);
          break;
        case "toggleShowAll":
          setShowAllLayers((v) => !v);
          break;
        case "help":
        ...
        case "commit":
          ...
          break;
        case "delete":
          // Backspace trims the draft first; otherwise delete the selection.
          if (e.key === "Backspace" && sliceDraft) {
            if (!dropLastDraftPoint()) return;
          } else if (mode === "edit") {
            if (!deleteSelectedAnnotation()) return;
          } else {
            return;
          }
          break;
        ...
        <div className="flex flex-wrap items-stretch justify-start gap-x-5 gap-y-2 border-b border-[color:var(--color-ocean-green)]/20 bg-[color:var(--color-surface-2)] px-4 py-1 text-center">
          ...
          <div className="flex flex-col items-center justify-between gap-0.5">
            ...
            <div className="flex items-center gap-1">
              ...
              />
              {mode === "polygon" || mode === "layer" || mode === "spline" ? (
                <ToolbarButton
                  text="Finish"
                  ...
                  title={
                    mode === "layer"
                      ? `Finish layer line (Enter) — ${
                          sliceDraft?.type === "layer" ? sliceDraft.points.length : 0
                        }/${LAYER_MIN_POINTS} points minimum`
                      : mode === "spline"
                        ? `Finish spline and name it (Enter) — ${
                            sliceDraft?.type === "spline" ? sliceDraft.points.length : 0
                          }/${SPLINE_MIN_POINTS} points minimum`
          ...
          <div className="flex flex-col items-center justify-between gap-0.5">
            ...
            <div className="flex items-center gap-1">
              ...
              <ToolbarButton
                ...
                title="Line (3) — click two points for a straight segment"
              />
              <ToolbarButton
                text="Layer"
                icon={<Waves className="h-5 w-5" />}
                tone={mode === "layer" ? "accent" : "default"}
                onClick={() => setMode("layer")}
                title={`Layer (4) — click ${LAYER_MIN_POINTS}+ points across the image; Enter or double-click to finish. The line extends to both edges.`}
              />
              <ToolbarButton
                text="Spline"
                icon={<Spline className="h-5 w-5" />}
                tone={mode === "spline" ? "accent" : "default"}
                onClick={() => setMode("spline")}
                title={`Spline (5) — click ${SPLINE_MIN_POINTS}+ points for a smooth named curve; Enter or double-click to finish and name it`}
              />
              <ToolbarButton
                text="Freehand"
                ...
                title="Freehand (6) — drag to draw; release to finish"
              />
              <ToolbarButton
                text="Pan"
          ...
          <div className="flex flex-col items-center justify-between gap-0.5">
            ...
            <div className="flex items-center gap-1">
              <ToolbarButton
                text={saving ? "Saving…" : saveSuccess ? "Saved" : "Save"}
                icon={<Save className="h-5 w-5" />}
                onClick={() => void saveProjectToDb()}
                tone={saveSuccess ? "accent" : "default"}
                disabled={saving}
                title={
                  saving
                    ? "Saving…"
                    : "Save now (Ctrl+S) — auto-save runs 2s after each change"
                }
              />
              <ToolbarButton
                text="Save As"
              ...
              <ToolbarButton
                ...
                onClick={() => {
                  // Await the flush: the Measurements view reads the SAVED project,
                  // so navigating first would show it stale annotations.
                  void (async () => {
                    await saveProjectToDb({ silent: true });
                    const q = selectedFileName
                      ? `?file=${encodeURIComponent(selectedFileName)}`
                      : "";
                    navigate(`/projects/${id}/measurements${q}`);
                  })();
                }}
                disabled={!project}
                title="Measure layer thickness"
              />
              <ToolbarButton
                text="Help"
